...
from skimage import color # CIEDE 2000 계산용
...
def calculate_ciede2000(img1, img2):
    """
    두 이미지 간의 CIEDE 2000 색상 차이 평균을 계산합니다.
    img1, img2: [H, W, 3] numpy array (0~255)
    """
    # 0~1 사이 값으로 정규화 후 Lab 변환
...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        if epoch % opt.save_epoch_freq == 0:              # cache our model every <save_epoch_freq> epochs
            ...
            # [추가] loss_log.txt에 검증 결과 직접 기록
           
            message = f'(VALIDATION) epoch: {epoch}, SSIM: {mtpfm_val["SSIM"]:.4f}, PSNR: {mtpfm_val["PSNR"]:.4f}, MSE: {mtpfm_val["MSE"]:.6f}\n'
            with open(os.path.join(opt.checkpoints_dir, opt.name, "loss_log.txt"), "a") as log_file:
