...
class PFAN(nn.Module):
    def __init__(self, *, input_nc, output_nc, ngf, hidden_dim, layers, heads, channels=3, num_classes=1000, head_dim=32, window_size=8,
        ...
        model_1 = [
                 # 주의: 여기서 input_nc는 외부에서 객체 생성 시 반드시 '4'로 전달되어야 합니다!
        ...
        # [삭제] 기존의 복잡했던 중간 융합용 완충 계층(self.dcp_buffer)은 이제 필요 없습니다.
        
        self.model_3 = nn.Sequential(*model_3)
        self.model_3_1 = nn.Sequential(*model_3_1)
        
    def forward(self, img, D_refined):
        # 1. [핵심] Early Fusion: RGB 이미지(3채널) + 정제된 DCP 맵(1채널) 병합
        # img: (B, 3, H, W) / D_refined: (B, 1, H, W)
        img_with_dcp = torch.cat([img, D_refined], dim=1) # 채널(dim=1) 기준으로 합쳐 4채널 텐서 생성
        
        # 2. 4채널 입력을 네트워크의 초기 특징으로 변환
        x = self.model_1(img_with_dcp)
        
        # 3. 고주파 특징 추출 (MBI 블록)
        x1 = self.convnext1(x)
        x_hf = self.convnext2(x1)

        # 4. 저주파 특징 추출 (LAT 트랜스포머 블록)
        x_lf = self.vit(x_hf)
        
        # [삭제] 내부에서 억지로 곱해주던 이전 DCP-SA 수식 전체 폐기
        
        # 5. 기존 네트워크의 정상적인 텐서 결합 (고주파 + 저주파 + 초기 잔차)
        x2 = x_hf + x_lf + x
