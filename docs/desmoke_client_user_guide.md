...
```
┌─ Header card (다크 카드형 컨트롤바) ────────────────────────────────────────────┐
│ INPUT          │ DEVICE          │ DETECTOR             │ ACTIONS               │
│ [📷Camera0  ▾] │ [⚡GPU(cuda:0)▾] │ ☑ Smoke detect       │ [▶Start][⏹Stop]       │
│                │                 │ ☐ Only when smoke    │ [⏺Rec][📷Snap]        │
└─────────────────────────────────────────────────────────────────────────────────┘
[ LIVE | ANALYSIS ]   ← 탭 전환
┌─────────────────────────────┐  ┌─────────────────────────────┐
│ ◉ ORIGINAL                  │  │ ✓ DESMOKED                  │
└─────────────────────────────┘  └─────────────────────────────┘
  (ANALYSIS 탭은 2×2: 원본 / DCP map / 연기 분포 히트맵 / 디스모킹)

[● LIVE] [▲ SMOKE · thin]  [FPS 14.2] [Latency 67ms] [YOLO 0.72] [Frames 1247]
```

### 4-1. 입력 소스 선택 (Source)

INPUT 콤보박스에서 입력 소스를 선택합니다. 자동으로 다음 항목들이 채워져 있습니다.

- **📷 Camera 0 / N** : 자동 탐지된 카메라·캡처카드. USB 캡처카드 연결 시 보통 0 또는 1.
- **📁 (마지막 사용 파일명)** : 직전 세션에서 사용한 비디오 파일이 있으면 자동 등록.
- **📁 비디오 파일 선택...** : 누르는 즉시 **파일 다이얼로그가 열림**. 고르면 새 항목으로 추가되고 바로 선택됨. 다른 파일을 더 고르면 콤보 안에 누적되어 재사용 가능.
...
복강경 수술 영상에서 연기 발생 구간을 자동으로 판정합니다. `feature/detection` 브랜치의
`video_inference_v3` 알고리즘을 단일 클래스로 통합한 결과로, 다음 4단계 파이프라인이 매 프레임 실행됩니다.

```
YOLOv8n-cls → Sliding Vote(5/4) → BalloonGate(흰 기구 억제) → ThinSmoke v2 (얇은 연기)
                                 ↓
                            Hysteresis + Cooldown 상태 머신
```

| 토글 | 의미 |
|---|---|
| **Smoke detect** | ON 시 매 프레임 탐지 수행, 상태바에 `▲ SMOKE` / `● CLEAR` pill 표시 |
| **Only when smoke** | ON 시 SMOKE 판정 구간에서만 디스모킹 추론 실행 (절전 / 시연용) |

> **가중치 위치**: `runs/smoke_detector/yolov8n_cls/weights/best.pt`
> 환경변수 `SMOKE_DETECTOR_WEIGHTS` 로도 지정 가능.
> 가중치가 없으면 두 토글이 자동 비활성화됩니다 (디스모킹은 정상 동작).

### 4-5. 동작 버튼

| 버튼 | 동작 |
|---|---|
| **Start** | 워커 스레드 시작 → LIVE/ANALYSIS 프리뷰 시작 |
| **Stop** | 워커 정상 종료 (녹화 중이면 mp4 마무리 저장) |
| **Snapshot** | 디스모킹 프리뷰의 **마지막 프레임**을 PNG/JPG 로 저장 |
...
| 탭 | 표시 내용 | 용도 |
|---|---|---|
| **LIVE** | 원본 ↔ 디스모킹 1×2 | 임상/일반 모니터링 |
| **ANALYSIS** | 원본 / DCP map / 연기 분포 히트맵 / 디스모킹 (2×2) | 시연 — 물리 파이프라인 전체를 한 화면에 |

- **DCP map (BONE 컬러맵)**: 어두운 채널 사전지식 (Dark Channel Prior + Guided Filter).
  밝을수록 짙은 연기. 256×256 으로 추론된 1채널 맵을 원본 해상도로 업스케일.
- **연기 분포 히트맵 (INFERNO 컬러맵)**: SurgiATM 물리 모델이 실제로 빼낸 연기량
  `(η + D_refined) / (η + 1) · (1 − ρ_DNN)`. 노랑일수록 강하게 제거됨.
...
|---|---|---|
| `● idle` | 대기 중 | 무채색 |
| `● LIVE` | 추론 진행 중 | **녹청** (Accent) |
| `⏺ REC + LIVE` | 추론 + 녹화 동시 | **빨강** |
| `⚠ error` | 오류 발생 | **빨강** (배경 어두움) |
| `○ DETECTOR OFF` | 탐지기 비활성/가중치 없음 | 무채색 |
| `● CLEAR` | 탐지기 ON · 연기 없음 | **녹청** |
| `▲ SMOKE` | 탐지기 ON · 연기 감지 (YOLO vote 통과) | **빨강** |
| `▲ SMOKE · thin` | ThinSmoke v2 보조로만 잡힌 얇은 연기 | **빨강** |
| `▲ SMOKE · cd` | Cooldown 강제 유지 구간 (0.5초) | **빨강** |
| `▲ SMOKE ⚪` | SMOKE 와 동시에 흰 기구 감지 | **빨강** |
| `FPS` | 1초 윈도우 평균 처리 속도 | — |
| `Latency` | 마지막 프레임 추론 소요 시간 | <70ms 녹청 / <150ms 노랑 / 그 이상 빨강 |
| `YOLO conf` | 마지막 프레임의 YOLO smoke 신뢰도 | <0.4 무채색 / <0.65 노랑 / 그 이상 빨강 |
| `Frames` | 누적 처리 프레임 수 | — |
...
1. `python -m app.main`
2. INPUT 콤보 → **📁 비디오 파일 선택...** → 다이얼로그에서 `assets/capture1.avi` 선택
   (선택 즉시 `📁 capture1.avi` 항목이 콤보에 추가되고 자동으로 활성화됨)
3. Device: **⚡ GPU (cuda:0)**
4. **⏺ Record** 토글 ON (버튼이 빨간색으로 바뀜)
5. **▶ Start** 클릭 → 저장 위치 다이얼로그에서 `desmoke_capture1.mp4` 입력
6. 상태바가 `⏺ REC + LIVE` 로 바뀌고 추론 시작
7. 영상이 끝나면 자동으로 "재생이 끝났습니다" 다이얼로그 표시 + mp4 저장 완료
...
│   ├── main_window.py                ← PySide6 메인 윈도우 (다크 테마 + 카드 레이아웃)
│   ├── style.py                      ← 글로벌 QSS 스타일시트 + 색상 토큰
